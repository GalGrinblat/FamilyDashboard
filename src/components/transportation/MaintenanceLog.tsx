                ...
                {allLogs.length === 0 ? (
                    ...
                    <div className="space-y-4 mt-4">
                        {allLogs.map((log, index) => (
                            <div key={log.id || index} className="flex justify-between items-center text-sm border-b border-zinc-100 dark:border-zinc-800 pb-3 last:border-0 last:pb-0 group">
                                <div className="flex flex-col">
                                    <span className="font-medium flex items-center gap-2">
                                        {typeIcons[log.type] || typeIcons['other']}
                                        {log.description}
                                        <span className="text-xs bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded-full mr-2">{log.carName}</span>
                                    </span>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <Button variant="ghost" size="icon" onClick={() => handleDelete(log.carId, log.id)}>

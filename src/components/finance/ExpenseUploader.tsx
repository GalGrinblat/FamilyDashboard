...
export function ExpenseUploader({ categories, accounts }: { categories: { id: string, name_he: string, domain?: string }[], accounts: { id: string, name: string }[] }) {
                ...
                <div className="relative space-y-4">
                    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 shadow-sm flex items-center justify-between gap-4">
                        ...
                        <select
                            ...
                            dir="rtl"
                        >
                            <option value="" disabled>-- בחר חשבון / כרטיס --</option>
                            {accounts.map(acc => (
                    ...
                    <div className={!selectedAccountId ? "opacity-50 pointer-events-none transition-opacity" : "transition-opacity relative"}>

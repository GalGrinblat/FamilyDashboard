...
export function AddEditPensionDialog({ triggerButton, assetToEdit }: AddEditPensionDialogProps) {
    ...
    const [severancePercent, setSeverancePercent] = useState(metadata.severance_percent?.toString() || "")
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                {triggerButton || (
                    <Button variant={isEditing ? "ghost" : "default"} size={isEditing ? "icon" : "default"}>
                        {isEditing ? <Pencil className="h-4 w-4" /> : <><Plus className="ml-2 h-4 w-4" /> קופה חדשה</>}
                    </Button>
                )}
            </DialogTrigger>
            <DialogContent className="sm:max-w-[425px]" dir="rtl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {isEditing ? 'עדכן את יתרת הקופה ואחוזי ההפרשה.' : 'הוסף קרן פנסיה, ביטוח מנהלים או קרן השתלמות למעקב סך החסכונות.'}
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleSubmit} className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="name" className="text-right whitespace-nowrap">שם הקופה/קרן</Label>
                        <Input
                            id="name"
                            value={name}
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="pensionType" className="text-right">סוג</Label>
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="value" className="text-right">צבירה נוכחית</Label>
                        <Input
                            id="value"
                            type="number"
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="employeePercent" className="text-right text-xs whitespace-nowrap">הפרשת עובד (%)</Label>
                        <Input
                            id="employeePercent"
                            type="number"
                            step="0.01"
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="employerPercent" className="text-right text-xs whitespace-nowrap">הפרשת מעסיק (%)</Label>
                        <Input
                            id="employerPercent"
                            type="number"
                            step="0.01"
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="severancePercent" className="text-right text-xs whitespace-nowrap">הפרשת פיצויים (%)</Label>
                        <Input
                            id="severancePercent"
                            type="number"
                            step="0.01"

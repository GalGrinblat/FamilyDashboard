    ...
    <Card className="hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors relative overflow-hidden group">
      <div
        className={`absolute top-0 right-0 w-1.5 h-full ${
          policy.type === INSURANCE_TYPES.HEALTH
            ? 'bg-rose-500'
            : policy.type === INSURANCE_TYPES.PROPERTY
              ? 'bg-emerald-500'
              : 'bg-blue-500'
        }`}
      />
        ...
        <div className="grid grid-cols-2 gap-y-2 mt-4 text-lg text-zinc-600 dark:text-zinc-400">
          {policy.policy_number && (
            <div className="flex items-center gap-1.5">
              ...
              <span>
                פוליסה:{' '}
                <span className="font-medium font-mono text-zinc-700 dark:text-zinc-300 ml-1">
                  {policy.policy_number}
                </span>
              </span>
            </div>
          ...
          {policy.renewal_date && daysUntilRenewal !== null && (
            <div
              className={`flex items-center gap-1.5 col-span-2 mt-1 px-2 py-1.5 rounded-md ${renewalBg} ${renewalColor}`}
            >

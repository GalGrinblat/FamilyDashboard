                ...
                {contracts.length === 0 ? (
                    ...
                    <div className="space-y-4 mt-4">
                        {contracts.map(contract => {
                            ...
                            return (
                                <div key={contract.id} className="flex justify-between items-center text-sm border-b border-zinc-100 dark:border-zinc-800 pb-3 last:border-0 last:pb-0 group">
                                    <div className="flex flex-col">
                                        <span className="font-medium">{contract.name}</span>
                                        <span className={`text-xs ${isExpired ? 'text-red-500 font-semibold' : 'text-muted-foreground'}`}>
                                            {contract.end_date ? `עד: ${new Date(contract.end_date).toLocaleDateString('he-IL')}` : 'ללא תאריך סיום'}
                                            {isExpired ? ' (פג תוקף)' : ''}
                                        </span>
                                    </div>

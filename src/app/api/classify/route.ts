...
const classificationSchema: Schema = {
  ...
  items: {
    ...
    properties: {
      index: {
        type: Type.INTEGER,
        description: 'Index of the transaction in the provided unmapped list',
      },
      suggested_category_id: {
        type: Type.STRING,
        description: 'UUID of the matched existing category (if any). Send null if none match.',
      },
      suggested_new_category: {
        type: Type.OBJECT,
        description:
          'If no existing category matches well, provide a suggestion for a NEW category to create.',
        nullable: true,
        properties: {
...
export async function POST(req: Request) {
  try {
    ...
    const mappingsDict = new Map(
      mappings.map((m) => [m.raw_merchant_string, m.mapped_category_id]),
    );
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        unmapped: finalResults.filter((r) => !r.suggested_category_id && !r.suggested_new_category)
          .length,

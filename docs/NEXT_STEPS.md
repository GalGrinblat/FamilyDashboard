...
### 3.1 PWA Setup
- Add `public/manifest.json` with app name, icons, theme color, and `"display": "standalone"`.
- Register a service worker (use `next-pwa` package or a custom `public/sw.js`).
- Enable offline caching for the **Overview** (`/`) page. (A Shopping List page is planned for Phase 2 and will need offline caching added at that time.)
